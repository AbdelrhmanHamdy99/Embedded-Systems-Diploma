...
boolean LCD_Busy(void){
	...
	int8_t i; /*iterator*/
	DIO_PinConfig_T pinConfig; /*a structure to hold pins configurations*/

	/*Change direction of data bins to be input*/
	pinConfig.pinMode = DIO_PinMode_In_Floating;
	for(i = LCD_TotDataPins - 1; i >= LCD_TotDataPins - LCD_Interface;i--){
		pinConfig.pinNumber   = LCD_DataPins[i];
		MCAL_DIO_Init(LCD_Data_Port, &pinConfig);
	}
	...
	MCAL_DIO_WritePin(LCD_Control_Port, RS, DIO_PinValue_Low);/*Instruction*/
	MCAL_DIO_WritePin(LCD_Control_Port, RW, DIO_PinValue_High);/*read*/


	_delay_us(0.05); /*TAS = 50ns (min = 40ns)*/
	MCAL_DIO_WritePin(LCD_Control_Port, E, DIO_PinValue_High); /*Enable the LCD*/
	_delay_us(0.2); /*tDDR (wait until data is ready) = 200ns (min = 160ns)*/
	busy = MCAL_DIO_ReadPin(LCD_Data_Port, DB7); /*read busy flag*/
